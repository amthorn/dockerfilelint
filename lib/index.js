...
module.exports.run = function ({ configPath, content, customRulesetPath, configContents, ruleContents }) {
  ...
  var state = {
    ...
    customRuleset: ruleContents || loadCustomRuleset(customRulesetPath)
  }
...
function runLine(state, instructions, idx) {
  ...
  if (state.customRuleset){
    ...
    for(const rule of Object.entries(state.customRuleset)){
      ...
      if (line === state.lines.length - 1 && rule[1].finalizer) {
